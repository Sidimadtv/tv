  ...
  const SESSION_KEY = 'blog_auth_session';
      const SESSION_DURATION = 3 * 60 * 60 * 1000; // 1 hour
      ...
      function showScreen(screenId) {
        [loginScreen, signupScreen, forgotScreen, otpScreen, resetScreen].forEach(screen => {
      ...
      async function makeFetchRequest(action, data) {
        ...
        console.log('✔️ Fetch result:', result);
        return result;
      }
      ...
      function makeJSONPRequest(action, data) {
        return new Promise((resolve, reject) => {
          ...
          window[callbackName] = function(result) {
            cleanup();
            console.log('✔️ JSONP result:', result);
            resolve(result);
          };
        ...
        try {
          ...
          try {
            ...
            if (response.ok) {
              ...
              console.log('✔️ Server connection successful (fetch):', data);
              return true;
            }
          } catch (fetchError) {
          ...
          const testResult = await new Promise((resolve) => {
            ...
            window[callbackName] = function(data) {
              cleanup();
              console.log('✔️ Server connection successful (JSONP):', data);
              resolve(true);
            };
      ...
      document.addEventListener('DOMContentLoaded', async function() {
        ...
        if (isAuthenticated) {
          console.log('✔️ User authenticated from stored session');
